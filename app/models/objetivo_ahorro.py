...
class ObjetivoAhorro(db.Model):
    ...
    def validar(self):
        ...
        if self.fecha_limite and self.fecha_inicio > self.fecha_limite:
    ...
    def progreso(self):
        ...
        progreso = (float(self.cantidad_actual) / float(self.cantidad_objetivo)) * 100
    ...
    def cantidad_restante(self):
        ...
        """
        restante = float(self.cantidad_objetivo) - float(self.cantidad_actual)
        return max(restante, 0)  # No negativa
    
    @property
    def esta_completado(self):
        """Verifica si el objetivo está completado"""
        return float(self.cantidad_actual) >= float(self.cantidad_objetivo)
    ...
    def añadir_cantidad(self, cantidad):
        ...
        nueva_cantidad = float(self.cantidad_actual) + cantidad
    ...
    def resumen_objetivos(cls, usuario_id):
        ...
        total_actual = sum(float(o.cantidad_actual) for o in activos)
